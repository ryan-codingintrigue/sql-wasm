...
const llvmSqlite = "emcc -DSQLITE_OMIT_LOAD_EXTENSION -DSQLITE_DISABLE_LFS -DLONGDOUBLE_TYPE=double -DSQLITE_THREADSAFE=0 -DSQLITE_ENABLE_FTS3 -DSQLITE_ENABLE_FTS3_PARENTHESIS src/native/sqlite3.c src/native/extension-functions.c -o src/native/compiled/sqlite3.bc";
...
const emcc = `emcc -s WASM=1 -s MODULARIZE=1 -s EXPORT_ES6=1 -s 'EXPORT_NAME="sqlite3"' -s RESERVED_FUNCTION_POINTERS=64 -s FORCE_FILESYSTEM=1 -s "EXPORTED_FUNCTIONS=${exportedFunctions}" -s "EXTRA_EXPORTED_RUNTIME_METHODS=${exportedRuntimeMethods}" src/native/compiled/sqlite3.bc -o src/native/compiled/sqlite3.js`;
